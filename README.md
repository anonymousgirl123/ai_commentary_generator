...
Scene Understanding (Gemini 2.5 Flash – Vision LLM)
Commentary Generation (Gemini 2.5 Pro – Language LLM)
...
▶️ Usage 🖥️ Option A – Interactive Dashboard (Recommended) python dashboard.py

📌 Dashboard Flow

Step 1 – Video & API Setup Enter full video path Enter API key (skip if env variable is set)
...
📤 Output Terminal → Timestamped commentary commentary_output.txt → Full script scene_analysis.json → (optional) intermediate data Audio file → (if TTS enabled)

🤖 Models Used Gemini 2.5 Flash → Vision (frame analysis) Gemini 2.5 Pro → Language (commentary generation)
